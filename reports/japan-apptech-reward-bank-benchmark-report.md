...
일본에서 은행송금을 제공하려면 은행명과 지점명/지점코드를 같이 선택받는 형태가 맞다. 계좌유형은 화면에 명확히 표시하되 기본은 `普通預金(일반/보통예금)`으로 두고, `当座預金(당좌/수표성 계좌)`, `貯蓄預金(저축예금)`은 필요 시 선택 옵션으로 제공하는 것이 좋다. 다만 초기 앱테크 리워드는 은행송금보다 PayPay/Amazonギフト/주요 포인트 교환을 먼저 제공하는 사례가 더 많다.
...
- `貯まったコインは特典と交換可` 메시지와 함께 PayPay, Amazonギフト, dポイント, Ponta, 楽天ポイント, Vポイント, 쿠폰형 상품이 함께 노출된다.
...
```mermaid
flowchart TD
  A["이동/레시트/서비스 이용으로 코인 적립"] --> B["コイン交換"]
  B --> C["교환처 리스트 확인"]
...
- PayPay 공식 기사에서 TikTok Lite는 동영상 시청만으로 포인트를 모으고 PayPayポイント 등으로 교환 가능한 앱으로 소개된다.
- 일본에서는 “현금”보다 `PayPay/楽天/d/Ponta/Vポイント` 같은 생활권 포인트가 고객 체감 가치가 높다.
...
- 공개 도움말에 `주요 은행 버튼`, `기타 은행 검색`, `支店名を検索` 흐름이 안내된다.
...
|---|---|---|---|
| 리워드 체감 | 현금, 네이버페이, 카카오페이, 모바일상품권이 직관적 | PayPay, Amazonギフト, dポイント, Ponta, 楽天ポイント, Vポイント 등 생활권 포인트 체감이 큼 | 1차 MVP는 현금보다 즉시 쓸 수 있는 교환처를 앞에 배치 |
...
| ゆうちょ銀行 | 우체국 계좌도 일반 은행 계좌와 유사하게 처리 | ゆうちょ銀行은 기호/번호와 송금용 店名/預金種目/口座番号 변환 이슈가 있음 | ゆうちょ 선택 시 별도 안내와 변환 도움말을 보여줘야 함 |
...
| dポイント/Ponta/楽天/Vポイント | 외부 포인트 계정 ID 연동 또는 번호 입력, SMS/메일 확인 가능 | 최초 1회 연동 후 다음 교환부터 저장 계정 사용 |
...
- 상단: `現金に交換 / 현금으로 교환`
- 주의 문구: 본인명의 계좌만 가능, ゆうちょ銀行 예외 안내
- 주요 은행 빠른 선택: 楽天, PayPay, ゆうちょ, 三菱UFJ, 三井住友, みずほ
- `その他銀行を検索 / 기타 은행 검색`: 은행명을 히라가나/한자로 검색
- `支店名または支店コード / 지점명 또는 지점코드`: 일본 송금 필수 항목
- `口座種別 / 계좌종류`: 普通預金, 当座預金 등
- 기본값은 `普通預金(일반/보통예금)`, 当座/貯蓄은 필요 시 선택 옵션
- `口座番号 / 계좌번호`: 은행별 자리수 검증 필요
- `口座名義カナ / 계좌명의 카나`: 본인명의 일치 확인
- 수수료/완료 예정일 확인
- `入力内容を確認 / 입력내용 확인` CTA
...
일본 앱테크는 처음부터 은행계좌 입력을 넣기보다 PayPay, Amazonギフト, dポイント, Ponta, 楽天ポイント, Vポイント 같은 생활권 리워드를 먼저 제공하는 쪽이 자연스럽습니다. Cashwalk/Moneywalk뿐 아니라 トリマ, クラシルリワード, Powl, TikTok Lite, Uvoice 등도 기프트/포인트 교환 또는 DotMoney/디지털기프트 경유가 많습니다. 은행송금은 ハピタス, モッピー, ポイントタウン, GMOポイ活처럼 현금화 중심 서비스에서 쓰지만, 은행명+지점명+계좌번호+명의カナ 입력, 본인명의 제한, ゆうちょ例外, 송금 실패/조합 대응까지 필요해 MVP 후순위가 좋아 보입니다.

## 9. 운영/제휴 체크

| 질문 | 답변 | 설계 반영 |
|---|---|---|
| 은행 선택이 6개뿐인가? | 아니다. 6개는 주요 은행 빠른 선택 예시다. 실제로는 `その他銀行を検索`에서 은행명/금융기관코드로 검색해야 한다. | 주요 은행 버튼 + 전체 은행 검색 |
| PayPal인가? | 목업의 은행 버튼은 PayPal이 아니라 `PayPay銀行`이다. PayPay는 일본 간편결제/포인트 생태계, PayPay銀行은 은행이다. PayPal은 별도 글로벌 결제 서비스다. | 은행 버튼은 `PayPay銀行`로 표기 |
| 계좌종류가 필요한가? | 필요하다. 일본 송금은 `普通預金`, `当座預金` 등 계좌종류/預金種目을 요구하는 경우가 있다. 포인트사이트는 보통 普通預金만 허용하기도 한다. | 계좌종류 필드 노출, 기본값 普通預金 |
| 수수료가 있나? | 있을 수 있다. ハピタス처럼 직접 계좌입금에 교환 수수료를 명시하는 사례가 있고, 은행/교환처/대행사별로 다르다. | 교환 전 수수료와 완료 예정일 표시 |
| 본인확인은 어떻게 하나? | 서비스 로그인, SMS/메일 인증, 비밀번호/2FA, 외부 교환 플랫폼의 본인확인, 계좌명의カナ와 회원명 비교를 조합한다. PayPay 은행계좌 등록은 사전 本人確認이 필요하다고 안내한다. | 교환 신청 전 인증 단계와 명의 불일치 오류 처리 |
| 계좌 인증은 어떻게 하나? | 은행/지점코드 유효성, 계좌번호 자리수, 명의カナ 형식, 계좌체크 API/송금대행사 검증, 실제 송금 결과를 단계적으로 확인한다. 모든 은행에서 실시간 명의조회가 되는 전제로 보면 위험하다. | 검증 실패/송금 실패/반려 상태값 운영 |

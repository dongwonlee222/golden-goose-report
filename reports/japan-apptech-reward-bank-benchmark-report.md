...
- 상단: `現金に交換 / 현금으로 교환`
- 주의 문구: 본인명의 계좌만 가능, ゆうちょ銀行(우체국은행) 예외 안내
- 주요 은행 빠른 선택: 楽天(라쿠텐), PayPay銀行(PayPay은행), ゆうちょ(우체국), 三菱UFJ(미쓰비시UFJ), 三井住友(미쓰이스미토모), みずほ(미즈호)
- `その他銀行を検索 / 기타 은행 검색`: 은행명을 히라가나/한자로 검색
- `支店名または支店コード / 지점명 또는 지점코드`: 일본 송금 필수 항목
- `口座種別 / 계좌종류`: 普通預金, 当座預金 등
- 기본값은 `普通預金(일반/보통예금)`, 当座(당좌/수표성 계좌)/貯蓄(저축예금)은 필요 시 선택 옵션

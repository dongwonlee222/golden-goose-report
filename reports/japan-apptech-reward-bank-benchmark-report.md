...
실제 사례에서 확인되는 핵심은 은행명만 받지 않고, 은행/금융기관과 지점 정보를 함께 선택하게 만드는 것이다. 다만 사용자 플로우는 단계별 화면으로 나누기보다 **출금 신청 한 화면에서 은행·지점·계좌정보를 모두 받고, 은행/지점 필드는 검색 바텀시트로 선택**하게 만드는 방식이 더 단순하다.

| 예시 | 확인된 UI/입력 패턴 | 우리 UI 반영 |
|---|---|---|
| GMOポイ活 | 주요 은행 버튼, 기타 은행 검색, `支店名(지점명)` 검색 안내 | 한 화면 폼 안에 은행/지점 선택 필드 배치, 탭하면 검색 바텀시트 |
...
- `銀行・金融機関 / 은행·금융기관`: 선택 필드. 탭하면 주요 은행 + 은행명/금융기관코드 검색 바텀시트
- `支店 / 지점`: 선택 필드. 탭하면 선택한 은행의 지점명/지점코드 검색 바텀시트

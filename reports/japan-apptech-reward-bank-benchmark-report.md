...
은행과 지점은 수가 많기 때문에 전체 드롭다운 나열이나 많은 은행 버튼은 피하고, `검색 우선` + `최근/추천 은행 최대 3개` + `지점명/지점코드 검색` 방식이 적합하다. Zengin-Net 통계는 2026년 3월 기준 금융기관 1,086개, 지점 29,010개를 집계하고 있으며, 대형은행은 지점이 300개를 넘는 사례도 확인된다.
...
flowchart TD
  ...
  C --> D["은행 검색/선택: 검색 우선 + 최근/추천 최대 3개"]
  D --> E["지점명/지점코드 검색"]
...
실제 사례에서 확인되는 핵심은 은행명만 받지 않고, 은행/금융기관과 지점 정보를 함께 선택하게 만드는 것이다. 다만 은행 수가 많기 때문에 사용자에게 은행 목록을 많이 보여주기보다 **검색 우선 UI**가 맞다. 출금 신청 한 화면에서 은행·지점·계좌정보를 모두 받고, 은행/지점 필드는 탭하면 바로 검색 입력에 포커스되는 선택 화면을 띄우는 방식이 단순하다.

| 예시 | 확인된 UI/입력 패턴 | 우리 UI 반영 |
|---|---|---|
| GMOポイ活 | 주요 은행 버튼, 기타 은행 검색, `支店名(지점명)` 검색 안내 | 한 화면 폼 안에 은행/지점 선택 필드 배치, 선택 화면은 검색 우선으로 구성 |
| モッピー / ハピタス | 현금 교환 시 로그인 후 은행 계좌 등록/확인, 수수료·완료일 안내 | 신청 버튼 위에 수수료/완료 예정일/본인명의 체크 표시 |
| ゆうちょ銀行(우체국은행) | 기호/번호가 아니라 송금용 `店名(지점명)`, `預金種目(예금종류)`, `口座番号(계좌번호)` 확인 필요 | ゆうちょ 선택 시 같은 화면 안에 변환 안내 문구 노출 |
...
- 상단: `現金に交換 / 현금으로 교환`, 도움말, 뒤로가기
- 안내: 본인명의 계좌만 가능, ゆうちょ銀行(우체국은행)은 송금용 지점명/계좌번호 필요
- `交換ポイント / 교환 포인트`: 교환할 포인트 입력
- `銀行・金融機関 / 은행·금융기관`: 선택 필드. 은행명을 검색해 선택하고, 금융기관코드는 자동 저장
- `支店 / 지점`: 선택 필드. 선택한 은행의 지점명/지점코드를 검색해 선택하고, 지점코드는 자동 저장
- `口座種別 / 계좌종류`: `普通預金(보통예금)` 기본 선택, `当座預金(당좌)`, `貯蓄預金(저축)`은 옵션
- `口座番号 / 계좌번호`: 보통 7자리 숫자, 은행별 형식 검증
- `口座名義カナ / 계좌명의 카나`: 통장/은행 앱의 카나 표기와 동일하게 입력
- 수수료/완료 예정일: 신청 버튼 바로 위에 표시
- 체크: 본인명의 계좌, 은행 등록 정보와 입력값 일치
- CTA: `入力内容を確認 / 입력내용 확인`
...
| 질문 | 답변 | 설계 반영 |
|---|---|---|
| 은행이 많아 선택이 어렵지 않나? | 맞다. 금융기관 수가 많아 전체 목록이나 많은 은행 버튼을 보여주면 부담이 크다. 기본은 은행명 검색으로 두고, 보조로 최근 선택/추천 은행을 최대 3개 정도만 노출하는 편이 낫다. | 검색 우선, 최근/추천 은행 최소 노출, 금융기관코드는 자동 저장 |
